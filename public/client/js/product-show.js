...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.changeMainImg = function (src, el) {
        ...
        if (!targetEl && src) {
            targetEl = document.querySelector(
                `.prod-thumb-item[data-img-src="${src}"]`,
            );
        }
        ...
        if (targetEl) {
            ...
            if (el && targetEl.dataset.variantName) {
                ...
                const matchedOptionBtn = document.querySelector(
                    `.btn-variant-option[data-group-index="0"][data-value="${firstPart}"]`,
                );
                if (
                    matchedOptionBtn &&
                    !matchedOptionBtn.classList.contains("active")
                ) {
    ...
    const hasVariants =
        variantsSection && variantsSection.dataset.hasVariants === "true";
    const totalGroupsCount = variantsSection
        ? parseInt(variantsSection.dataset.groupsCount || "1")
        : 0;
    ...
    window.onSelectVariantOption = function (btn, groupIndex, val) {
        ...
        if (btn.classList.contains("active")) {
            ...
            // Active nút mới
            if (row) {
                row.querySelectorAll(".btn-variant-option").forEach((b) =>
    ...
    function updateVariantState() {
        ...
        if (selectedGroupCount === totalGroupsCount && totalGroupsCount > 0) {
            ...
            for (let i = 0; i < totalGroupsCount; i++) {
                if (selectedOptionsByGroup[i])
                    selectedVals.push(selectedOptionsByGroup[i]);
            }
            ...
            if (activeMatchedVariant) {
                ...
                if (stockDisplay) {
                    stockDisplay.textContent = new Intl.NumberFormat(
                        "vi-VN",
                    ).format(stock);
                }
                if (buyQuantityInput) {
            ...
            if (selectedOptionsByGroup[0]) {
                ...
                const matchedVarWithImg = variantsData.find(
                    (v) => v.name.includes(optVal) && v.image_url,
                );
        ...
        fetch(`/products/${productId}/like`, {
            ...
            .then(function (data) {
                if (data.status === "success") {
                    if (countEl) {
                        countEl.innerText = new Intl.NumberFormat(
                            "vi-VN",
    ...
    window.addToCart = function (productId, isBuyNow) {
        ...
        const isGuest = detailContainer
            ? detailContainer.getAttribute("data-is-guest") === "true"
            : false;
        ...
        if (hasVariants && totalGroupsCount > 0) {
            ...
            if (selectedCount < totalGroupsCount || !activeMatchedVariant) {
                ...
                alert(
                    "⚠️ Vui lòng chọn đầy đủ Phân loại hàng (Màu sắc, kích cỡ...) trước khi mua!",
                );
                return;
            }
        }
        ...
        const cartStoreUrl = detailContainer
            ? detailContainer.getAttribute("data-cart-url")
            : "/cart/add";
        const cartIndexUrl = detailContainer
            ? detailContainer.getAttribute("data-cart-index-url")
            : "/cart";
            ...
            .then(function (data) {
                if (data.total_items !== undefined) {
                    ...
                    if (isBuyNow) {
                        ...
                        // Thêm vào giỏ hàng thành công: hiển thị toast hoặc thông báo
